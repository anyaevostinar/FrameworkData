...
short_data <- subset(subset(initial_data, update > 400000), partner == "Host")
...
ggplot(data=short_data,aes(x=update, y=task_count, group=task, colour=task)) + ylab("Task count") + xlab("Updates") + stat_summary(aes(color=task, fill=task),fun.data="mean_cl_boot", geom=c("smooth"), se=TRUE) + theme(panel.background = element_rect(fill='white', colour='black')) + theme(panel.grid.major = element_blank(), panel.grid.minor = element_blank()) + guides(fill=FALSE) +scale_colour_manual(name = "Hosts", values=viridis(9)) + scale_fill_manual(values=viridis(9)) + facet_grid(task ~ ., scales='free')
